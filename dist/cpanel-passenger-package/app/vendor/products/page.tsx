...
import { parseProductOptionGroups, parseProductSpecifications } from '@/lib/product-options'
              ...
              <div key={product.id} className="bg-white rounded-lg shadow overflow-hidden hover:shadow-lg transition">
                ...
                <div className="p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {parseProductOptionGroups(product.optionGroupsJson)
                      .slice(0, 2)
                      .map((group) => (
                        <span
                          key={group.id}
                        ...
                        >
                          {group.name}: {group.values.length} options
                        </span>
                      ))}
                    {parseProductSpecifications(product.specificationsJson).length ? (
                      <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-700">
                        {parseProductSpecifications(product.specificationsJson).length} specs
                      </span>
                    ) : null}
                  </div>
