...
import { ArrowLeft, CheckCircle2, Clock3, Download, Mail, MapPinOff, Phone, Receipt, Store, Wallet } from 'lucide-react'
...
export default async function AdminVendorDetailPage({
  ...
  const requestId = createRequestDebugId('admin-vendor-page')
  let vendor
  ...
  try {
    ...
    logRequestDebug('admin vendor page start', requestId, {
      ...getSessionDebug(session),
      vendorId: id || null,
    })

    if (!session?.user) {
      logRequestDebug('admin vendor page redirect signin', requestId, {
        vendorId: id || null,
      })
    ...
    if (role !== 'admin') {
      logRequestDebug('admin vendor page redirect home', requestId, {
        ...getSessionDebug(session),
        vendorId: id || null,
      })
      redirect('/')
    ...
    if (!id) {
      logRequestDebug('admin vendor page missing id', requestId, {})
      notFound()
    }
    ...
    if (!vendor || !vendor.isVendor) {
      logRequestDebug('admin vendor page not found', requestId, {
        vendorId: id,
      })
      notFound()
    }

    logRequestDebug('admin vendor page success', requestId, {
      vendorId: id,
      vendorName: vendor.vendorName || null,
    })
  } catch (error) {
    logRequestError('admin vendor page failed', requestId, error, {})
    throw error
  }
  ...
  const requestedPayouts = vendor.payouts
    .filter((payout) => payout.status === 'requested')
    .reduce((sum, payout) => sum + payout.amount, 0)
  const deductedPayouts = vendor.payouts
    .filter((payout) => ['approved', 'processing', 'completed'].includes(payout.status))
    .reduce((sum, payout) => sum + payout.amount, 0)
  const ledgerBalance = Math.max(0, completedEarnings - deductedPayouts)
  const availableBalance = Math.max(0, ledgerBalance - requestedPayouts)
      ...
      <div className="flex items-start gap-3">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <span className="inline-flex rounded-full bg-blue-50 px-2.5 py-1 text-[11px] font-semibold text-blue-700">
                Marketplace markup {vendor.commissionRate ?? 10}%
              </span>
            </div>
            <div>
              <div className="flex flex-wrap gap-2">
                ...
                <a
                  href={`/api/admin/vendors/${vendor.id}/statement`}
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(145px,1fr))] gap-2">
          <div className="rounded-xl bg-slate-50 px-3 py-2.5">
            ...
            <p className="mt-1.5 text-xl font-bold text-slate-900">{formatMoney(availableBalance)}</p>
            {requestedPayouts > 0 ? (
              <p className="mt-1 text-[11px] text-slate-500">{formatMoney(requestedPayouts)} awaiting approval</p>
            ) : null}
          </div>
          <div className="rounded-xl bg-slate-50 px-3 py-2.5">
        ...
        <div className="space-y-4">
          ...
          <section className="rounded-[1.35rem] border border-slate-200 bg-white shadow-sm">
            ...
            <div className="px-4 py-5 text-center">
              ...
              <p className="mt-2 text-xs text-slate-500">Percentage added to customer pricing for this vendor</p>
            </div>
          </section>
          ...
          <section className="rounded-[1.35rem] border border-slate-200 bg-white shadow-sm">
            ...
            <div className="space-y-2.5 px-4 py-4 text-xs text-slate-600">
              <div className="flex items-start gap-2.5">
                ...
                <p>Account balance now acts as the vendor ledger balance after paid sales are credited.</p>
              </div>
              <div className="flex items-start gap-2.5">
                <Receipt className="mt-0.5 h-4 w-4 text-blue-600" />
                <p>Approved payout requests deduct the requested amount from the vendor balance for settlement tracking.</p>
              </div>
              <div className="flex items-start gap-2.5">
