...
import { Bell, DollarSign, PackageCheck, ShoppingCart, TrendingUp, Truck, Users } from 'lucide-react'
...
function getOrderLocation(order: AdminOrder) {
...
export default function AdminDashboard() {
  ...
  const lateItems = useMemo(
    () => orders.flatMap((order) => order.items.filter((item) => item.lateDelivery?.isLate)),
    [orders]
  )
  const proofItems = useMemo(
    () => orders.flatMap((order) => order.items.filter((item) => item.proof)),
    [orders]
  )
    ...
    <div className="space-y-4">
      <div className="theme-panel overflow-hidden rounded-[1.35rem]">
        <AdminPageHeader
          ...
          description="Monitor vendor orders, fulfillment pressure, customer updates, and the live operations mix."
          action={
            <button
        ...
        <div className="space-y-4 p-3.5 sm:p-4">
          <div className="grid grid-cols-[repeat(auto-fit,minmax(145px,1fr))] gap-2">
            ...
            <AdminStatCard icon={Truck} label="Late Items" value={lateItems.length} helper="Order items currently beyond ETA" />
            <AdminStatCard icon={PackageCheck} label="Proof Logged" value={proofItems.length} helper="Items with submitted delivery proof" />
          </div>

          <div className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
            <AdminSectionCard
              title="Operations Pulse"
              description="A quick read of where the current fulfillment pressure is concentrated."
            >
              <div className="grid gap-3 md:grid-cols-3">
                <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Order mix</p>
                  <div className="mt-3 space-y-2 text-sm">
                    <div className="flex items-center justify-between rounded-2xl bg-white px-3 py-2">
                      <span className="text-slate-600">Pending</span>
                      <span className="font-semibold text-slate-900">{statusCounts.pending}</span>
                    </div>
                    <div className="flex items-center justify-between rounded-2xl bg-white px-3 py-2">
                      <span className="text-slate-600">Accepted</span>
                      <span className="font-semibold text-slate-900">{statusCounts.accepted}</span>
                    </div>
                    <div className="flex items-center justify-between rounded-2xl bg-white px-3 py-2">
                      <span className="text-slate-600">In transit</span>
                      <span className="font-semibold text-slate-900">{statusCounts.courier_on_the_way}</span>
                    </div>
                  </div>
                </div>
                <div className="rounded-2xl border border-slate-200 p-4">
                  <p className="text-sm font-semibold text-slate-900">Completion health</p>
                  <p className="mt-2 text-sm text-slate-600">
                    {statusCounts.completed} orders have fully completed, while {statusCounts.declined + statusCounts.cancelled} have fallen out of the flow.
                  </p>
                </div>
                <div className="rounded-2xl border border-slate-200 p-4">
                  <p className="text-sm font-semibold text-slate-900">Customer updates</p>
                  <p className="mt-2 text-sm text-slate-600">
                    {notifications.length} recent admin notifications are visible, giving you the latest order and escalation messages in one place.
                  </p>
                </div>
              </div>
            </AdminSectionCard>

            <AdminSectionCard title="Attention Queue" description="Five compact items that usually deserve a manager look first.">
              <div className="grid gap-3">
                {filteredOrders.slice(0, 5).length === 0 ? (
                  <AdminEmptyState message="No orders are visible in the current queue." />
                ) : (
                    ...
                    <div key={order.id} className="rounded-2xl border border-slate-200 p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-slate-500">#{order.orderNumber || order.id.slice(0, 8)} | {order.user.email}</p>
                        </div>
                        <AdminBadge label={formatStatus(order.status)} tone={order.status === 'completed' ? 'green' : order.status === 'courier_on_the_way' ? 'blue' : order.status === 'accepted' ? 'amber' : order.status === 'declined' || order.status === 'cancelled' ? 'red' : 'neutral'} />
                      </div>
                      <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-600">
                        <span className="rounded-full bg-slate-100 px-3 py-1">${order.total.toFixed(2)}</span>
                        <span className="rounded-full bg-slate-100 px-3 py-1">{order.items.length} items</span>
                        <span className="rounded-full bg-slate-100 px-3 py-1">{getOrderLocation(order)}</span>
                      </div>
                    </div>
                  ))
                )}
            ...
            >
              <div className="space-y-4">
                {filteredOrders.length === 0 ? (
                  <AdminEmptyState message="No orders are visible for the current filter." />
                ) : (
                  filteredOrders.map((order) => (
                    <div key={order.id} className="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface-strong)] shadow-sm">
                      <div className="grid gap-4 bg-[var(--brand-accent-soft)]/40 px-4 py-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-start">
                        <div className="min-w-0">
                          <p className="text-[1.1rem] font-semibold text-gray-900">Order #{order.orderNumber || order.id.slice(0, 8)}</p>
                        ...
                        <div className="flex flex-col items-start gap-2 text-left md:items-end md:text-right">
                          <span className={`inline-flex rounded-full px-3 py-1 text-[11px] font-semibold ${getStatusClasses(order.status)}`}>
                            {formatStatus(order.status)}
                          </span>
                          <div className="space-y-1">
                            <p className="text-sm text-gray-700">
                              {order.fulfillmentMethod === 'pickup' ? 'Collection Address' : 'Delivery Address'}: {getOrderLocation(order)}
                            </p>
            ...
            <AdminSectionCard title="Status Notifications" description="Latest order status changes and admin alerts.">
              ...
              <div className="space-y-4">
                {notifications.length === 0 ? (
                  <AdminEmptyState message="No order status changes have been recorded yet." />
                ) : (
